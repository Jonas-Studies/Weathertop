import * as database from '../../database/database.js'

export default async function (reading) {
	try {
		const query = await database.query(
			'Insert into weathertop.readings ("ID", "weatherstation_ID", "weathercode", "temperature", "windspeed_in_kmh", "airpressure_in_hpa") values (gen_random_uuid(), $1::uuid, $2::integer, $3::float4, $4::float4, $5::float4)',
			[ reading.weatherstation_ID, reading.weathercode, reading.temperature.value, reading.windspeed.value, reading.airpressure.value ]
		)
		
		console.info("Inserted reading")
	}
	catch (error) {
		console.error("Could not insert reading")
		console.debug(error)
	}
}
